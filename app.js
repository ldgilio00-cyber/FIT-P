...
function resumeSessionFromHistory(sessionId){
  ...
  // ✅ Se sto riaprendo la *stessa* sessione attiva, NON azzerare dove ero rimasto.
  // (così posso uscire e rientrare senza ricominciare da capo)
  const same = (state.activeSessionId === s.id);
  ...
  // clamp indici (in caso di modifiche scheda/sessione)
  state.activeExIndex = Math.max(0, Math.min(state.activeExIndex||0, (s.items?.length||1)-1));
  const it = s.items?.[state.activeExIndex];
  const maxSet = (it?.sets?.length||1)-1;
  state.activeSetIndex = Math.max(0, Math.min(state.activeSetIndex||0, maxSet));
...
function deleteSessionById(sessionId, { closeModal = false } = {}){
  const s = state.sessions.find(x => x.id === sessionId);
  if(!s) return false;

  // se è la sessione attiva, chiudi overlay e resetta indici
  if(state.activeSessionId === s.id){
    syncActiveSetFromInputs();
  ...
  if(closeModal) closeSessionModal();
  return true;
}
...
(function bindSessionModal(){
  ...
  $("btnSessionModalDelete")?.addEventListener("click", ()=>{
    ...
    const ok = confirm("Eliminare questa sessione?");
    if(!ok) return;
    deleteSessionById(modalSessionId, { closeModal: true });
    toast("Sessione eliminata");
  });
})();
...
// ✅ Sicurezza: salva SEMPRE i valori correnti dagli input prima di cambiare serie/esercizio
// (su mobile può capitare che l'ultimo valore digitato non venga "committato" in tempo).
function syncActiveSetFromInputs(){
  const s = activeSession();
  if(!s) return;
  const it = s.items?.[state.activeExIndex];
  if(!it) return;
  const st = it.sets?.[state.activeSetIndex];
  if(!st) return;
...
function renderSingleSet(){
  ...
  const kgVal = String(st?.kg ?? "");
  const repsVal = String(st?.reps ?? "");
  ...
  $("singleSetBox").innerHTML=`
    <div class="singleCard">
      ...
      <div class="singleGrid">
        <label class="field">
          <span>Kg</span>
          <input inputmode="decimal" id="inKg" value="${kgVal}">
        </label>
        <label class="field">
          <span>Reps</span>
          <input inputmode="numeric" id="inReps" value="${repsVal}">
        </label>
  ...
  // fallback extra (alcuni browser mobile salvano solo al blur)
  $("inKg")?.addEventListener("change",(e)=>{ st.kg=e.target.value; saveState(); });
  $("inReps")?.addEventListener("change",(e)=>{ st.reps=e.target.value; saveState(); });
}
...
$("btnSessionExit")?.addEventListener("click", ()=>{
  // ✅ esci SENZA azzerare: posso riprendere più tardi dallo stesso punto
  syncActiveSetFromInputs();
  closeSessionUI();
  // non tocchiamo activeSessionId / indici
  saveState();
});
  ...
  sessions.slice(0,60).forEach(s=>{
    ...
    div.innerHTML=`
      ...
      <div class="row" style="justify-content:space-between; align-items:center; margin-top:8px;">
        ...
        <button class="iconBtn danger" title="Elimina sessione" data-session-del="1" data-session-id="${s.id}">🗑</button>
      </div>
    `;
    box.appendChild(div);
  });
...
function homeRefresh(){
  ...
  // ✅ CTA: se c'è una sessione APERTA, permetti di riprendere
  const btn = $("btnStartSession");
  const cur = activeSession();
  if(btn){
    btn.textContent = (cur && !cur.closed) ? "Riprendi allenamento" : "Inizia allenamento";
  }
...
$("btnStartDay")?.addEventListener("click", ()=>{
  ...
  if(cur && !cur.closed){
    openSessionUI();
    renderSession();
    toast("Ripresa sessione in corso");
    return;
  }
  const plan = getActivePlan();
